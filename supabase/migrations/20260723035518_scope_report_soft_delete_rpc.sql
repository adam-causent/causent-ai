-- Bind visible report removal to the session's selected workspace as well as
-- the report identity. Membership in another workspace must not make a forged
-- report id a valid target from the current workspace.

revoke all on function public.delete_decision_report_v1(uuid, uuid) from public;
drop function public.delete_decision_report_v1(uuid, uuid);
...
revoke all on function public.delete_decision_report_v1(uuid, uuid, uuid) from public;
grant execute on function public.delete_decision_report_v1(uuid, uuid, uuid)
