...
def test_total_and_ignores_leaked_direction(status, direction):
    ...
    b = belief_direction(_its(status, 123.4, direction, ci_low=100.0, ci_high=146.0))
    if status == "INSUFFICIENT":
        assert b == Belief(None, "INCONCLUSIVE")
    elif status != "OK":
        assert b == Belief(0.0, "INCONCLUSIVE")  # DEGENERATE/CONFOUNDED nuke direction
...
def test_noisy_zero_effect_agrees_with_scipy(seed):
    ...
    got = belief_direction(its_readout(series))
    oracle = _scipy_belief(series)
...
def test_belief_is_indifferent_to_magnitude():
    tiny = belief_direction(_its("OK", 0.001, "POSITIVE", ci_low=0.0001, ci_high=0.0019))
    huge = belief_direction(_its("OK", 1e9, "POSITIVE", ci_low=9e8, ci_high=1.1e9))
...
def test_no_input_mutation_and_stable():
    ...
    b1 = belief_direction(src)
    b2 = belief_direction(src)
